...
export default function MonitoringPOPage() {
  ...
  // --- Simpan fetchAll di luar agar bisa dipanggil ulang ---
    ...
    try {
      ...
      for (const itemId of selectedItemIdsToDelete) {
        ...
        if (mode === "permanent") {
          ...
          const poItem = poItemsData[itemId];
          if (!poItem) continue;
          await fetch(`http://localhost:5000/api/po-item/${itemId}`, {
            method: "DELETE",
          });
      ...
      if (mode === "restore") {
        for (const prId of prIdsToUpdate) {
          ...
          if (prRes.ok) {
            ...
            // --- Tambahkan log sebelum PUT ---
            console.log(
              "[MONITORING PO] Akan update status PR ke Gantung (frontend):",
              {
                prId,
                status: "Gantung",
                payload: { ...prData, status: "Gantung" },
              }
            );
