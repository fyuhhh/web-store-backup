...
export const logActivity = async (req, { action_type, entity_id, details, status }) => {
    try {
        ...
        const [result] = await db.query(
            `INSERT INTO activity_logs (id_user, nama_pengguna, action_type, entity_id, details, ip_address, user_agent) 
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [id_user, nama_pengguna, action_type, entity_id, detailStr, ip_address, user_agent]
        );
